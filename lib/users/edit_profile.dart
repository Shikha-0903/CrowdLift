...
class _EditProfileState extends State<EditProfile> {
  ...
  Future<void> fetchUserData() async {
    try {
      ...
      if (user != null) {
        _uid = user.uid;
        DocumentSnapshot userDoc = await FirebaseFirestore.instance.collection('crowd_user').doc(_uid).get();
  ...
  Future<void> removeProfileImage() async {
    ...
    try {
      ...
      // Update Firestore
      await FirebaseFirestore.instance.collection('crowd_user').doc(_uid).update({'profile_image': ""});
  ...
  Future<void> uploadImage(File imageFile) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('crowd_user').doc(_uid).update({'profile_image': downloadUrl});
            ...
            children: [
              GestureDetector(
                ...
                child: CircleAvatar(
                  ...
                  child: _imageUrl.isEmpty ? Icon(Icons.camera_alt, size: 40) : null,
                ),
              ),
              ...
              _buildTextField(
                _role.text == "Investor" ? "Investment Interest" : "Business Expectation",
                _interestExpect,
              ),
              _buildTextField(
                _role.text == "Investor" ? "Investment Capacity" : "Business Details",
                _capacityAbout,
              ),
              SizedBox(height: 20),
              ElevatedButton(
  ...
  Widget _buildReadOnlyField(String label, TextEditingController controller) {
    return Padding(
      ...
      child: ReusableTextField(controller: controller, hintText: label, readOnly: true),
    );
  }
