...
class NotificationService {
  ...
  Future<void> sendChatNotification({
    ...
    try {
      ...
      DocumentSnapshot userDoc = await _firestore.collection('crowd_user').doc(receiverId).get();
      ...
      if (userDoc.exists && userDoc.data() is Map<String, dynamic>) {
        ...
        print('Notification created for ${isReceiverOnline ? "online" : "offline"} user $receiverId');
      } else {
  ...
  Future<void> _processPendingNotifications() async {
    try {
      ...
      print('Processing ${pendingNotifications.docs.length} pending notifications');
  ...
  Future<void> _handleIncomingNotification(Map<String, dynamic> notification, String notificationId) async {
    try {
      // Create payload with just receiverId and receiverName
      Map<String, dynamic> payloadMap = {
        'receiverId': notification['senderId'], // sender becomes receiver when clicked
        'receiverName': notification['senderName'] // use sender name as receiver name
      };
      ...
      print('Displayed notification: ${notification['title']} with payload: $payloadJson');
    } catch (e) {
