import 'package:crowdlift/Authentication/log_out.dart';
import 'package:crowdlift/Authentication/login_screen.dart';
import 'package:crowdlift/about_app.dart';
import 'package:crowdlift/transaction/history/transaction_history.dart';
import 'package:crowdlift/transaction/receipt/payment_received.dart';
import 'package:crowdlift/users/my_profile.dart';
...
import 'dart:async';
import 'package:crowdlift/users/user_profile_screen.dart';
import 'users/role_based_data.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:crowdlift/utils/profile_avatar.dart';
import 'package:crowdlift/utils/search_users.dart'; // Import the new service
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<String> hintTexts = ["Search Investor,,", "Search Seeker..", "Search phone no..","Search email.."];
  ...
  Future<void> fetchUserName() async {
    try {
      ...
      if (_mounted) {
        setState(() {
          userId = "$uid";
          userName = (userDoc['name'][0].toUpperCase() + userDoc['name'].substring(1)) ?? "User";
          ...
          children: [
            DrawerHeader(
              decoration: const BoxDecoration(
                ...
                boxShadow: [BoxShadow(offset: Offset(2, 2), blurRadius: 4, color: Color(0xFFA998F7))],
              ),
              child: SizedBox(
                height: 300,
                child: Column(
            ...
            ListTile(
              ...
              hoverColor: Color(0xFFA998F7),
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => TransactionReceiptPage()));
              },
            ),
            ListTile(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              title: Text(
                (user['name'][0].toUpperCase() + user['name'].substring(1)) ?? 'Unknown',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white),
              ),
              subtitle: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("📞 ${user['phone'] ?? 'N/A'}", style: const TextStyle(color: Colors.white70)),
                  Text((user['role']=="Investor")?"🤵‍♂️ ${user['role'] ?? 'N/A'}":"👨‍💼 ${user['role'] ?? 'N/A'}", style: const TextStyle(color: Colors.white70)),
                  Text("📧 ${user['email'] ?? 'N/A'}", style: const TextStyle(color: Colors.white70)),
                ],
              ),
              onTap: () {
                Navigator.push(
          ...
          child: SingleChildScrollView(
            ...
            Text("Looking for investor? or seeker?", style: TextStyle(color: Colors.grey)),
            Padding(
                      ...
                      child: InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => FindData(query: "Investor"),
                            ),
                          );
                        },
                        borderRadius: BorderRadius.circular(12),
                      ...
                      child: InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => FindData(query: "Seeker"),
                            ),
                          );
                        },
                        borderRadius: BorderRadius.circular(12),
