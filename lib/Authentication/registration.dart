...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  Future<void> registerUser() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (phoneExists) {
        showCustomSnackBar(context, 'Phone number already exists. Please use another number.');
        return;
      }
      ...
      if (termsAccepted != null && termsAccepted) {
        try {
          ...
          FirebaseFirestore.instance.collection('crowd_user').doc(userCredential.user!.uid).set({
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => LoginScreen()), // Make sure LoginScreen is implemented
          );
          ...
          showCustomSnackBar(context, 'An error occurred during registration. Please try again.');
        }
      } else {
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        RadioListTile<String>(
                          title: Text("Seeker", style: TextStyle(color: Color(0xFF6750a4))),
                          value: "Seeker",
                        ...
                        RadioListTile<String>(
                          title: Text("Investor", style: TextStyle(color: Color(0xFF6750a4))),
                          value: "Investor",
                          groupValue: _selectedRole,
                          onChanged: (String? value) {
                            setState(() {
                    ...
                    SizedBox(height: 10),
                    ElevatedButton(
                      onPressed: registerUser, // Call registerUser function on press
                      child: Text("Register"),
                    ),
                    SizedBox(height: 10),
                    TextButton(
...
class TermsDialog extends StatefulWidget {
  ...
  const TermsDialog({required this.termsAccepted, required this.onAccept, Key? key})
      : super(key: key);
...
class _TermsDialogState extends State<TermsDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ElevatedButton(
          onPressed: termsAccepted ? () => Navigator.of(context).pop(true) : null,
          ...
          child: const Text('Accept & Continue', style: TextStyle(color: Colors.white, fontSize: 14)),
        ),
        TextButton(
