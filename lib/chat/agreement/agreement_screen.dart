...
class _AgreementFormScreenState extends State<AgreementFormScreen> {
  ...
  Future<void> _saveAgreementData({bool isSeeker = false, bool isInvestor = false}) async {
    try {
      Map<String, dynamic> updateData = {
        'seekerName': seekerNameController.text,
        'investorName': investorNameController.text,
        'businessName': businessNameController.text,
        'investmentAmount': double.tryParse(investmentAmountController.text) ?? 0.0,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text("Create Agreement",style: TextStyle(color: Colors.white),),backgroundColor: Color(0xFF070527),iconTheme: IconThemeData(color: Colors.white),),
      body: isLoading
            ...
            children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      buildTextField("Investor Name", investorNameController),
                      buildTextField("Business Name", businessNameController),
                      buildTextField("Investment Amount", investmentAmountController, keyboardType: TextInputType.number),
                      buildTextField("Duration (Years)", durationController, keyboardType: TextInputType.number),
                      buildTextField("Profit Sharing (Investor %)", profitSharingController1, keyboardType: TextInputType.number),
                      buildTextField("Profit Sharing (Seeker %)", profitSharingController2, keyboardType: TextInputType.number),
                      buildTextField("Terms & Conditions", termsController, maxLines: 4),
                    ],
                  ),
                ),
              ),
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      buildSwitchTile("Seeker Agreement Completed", seekerCompleted, (value) {
                        setState(() => seekerCompleted = value);
                        _saveAgreementData(isSeeker: true);
                      }, formatDate(seekerUpdatedAt)),
                      buildSwitchTile("Investor Agreement Completed", investorCompleted, (value) {
              ...
              Row(
                ...
                children: [
                  ElevatedButton(onPressed: agreementLocked
                      ? null
                      : (){
                    if (int.parse(profitSharingController1.text)>100 || int.parse(profitSharingController1.text)<=0 || int.parse(profitSharingController2.text)>100 || int.parse(profitSharingController2.text)<=0){
                      showCustomSnackBar(context, "profit sharing must be less than 100 and greater than 0");
                      return;
                    }else {
                      _saveAgreementData();
                    }
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: agreementLocked ? Colors.grey : Theme.of(context).primaryColor,
                      ...
                      if (_formKey.currentState!.validate()) {
                        if (int.parse(profitSharingController1.text)>100 || int.parse(profitSharingController1.text)<=0 || int.parse(profitSharingController2.text)>100 || int.parse(profitSharingController2.text)<=0){
                          ...
                          Agreement agreement = Agreement(
                            ...
                            investorName: investorNameController.text,
                            businessName: businessNameController.text,
                            investmentAmount: double.parse(investmentAmountController.text),
                            duration: durationController.text,
                            profitSharingI: profitSharingController1.text,
                            profitSharingS: profitSharingController2.text,
                            terms: termsController.text,
                          ...
                          showDialog(context: context, builder: (context) => GeneratePdfScreen(agreement: agreement, chatId: widget.chatId));
                        }
                      }
                    },
                    child: Text("Generate Agreement PDF",style: TextStyle(color: Colors.white),),
                    style: ElevatedButton.styleFrom(
  ...
  Widget buildTextField(String label, TextEditingController controller, {int maxLines = 1, TextInputType keyboardType = TextInputType.text}) {
    return Padding(
  ...
  Widget buildSwitchTile(String title, bool value, Function(bool) onChanged, String lastUpdated) {
    return Column(
      children: [
        SwitchListTile(title: Text(title), value: value, onChanged: agreementLocked ? null : onChanged),
        Text("Last Updated: $lastUpdated", style: TextStyle(color: Colors.grey)),
      ],
    );
  }
}
